  ...
  describe('deleteProductCategory', () => {
    ...
    describe('Happy Path', () => {
      it('should delete product category successfully with valid input', async () => {
        ...
        const result = await service.deleteProductCategory(validDeleteProductCategoryDto);

        expect(mockCallMicroservice).toHaveBeenCalledWith(
          mockProductClient.send(
            ProductPattern.DELETE_PRODUCT_CATEGORY,
            validDeleteProductCategoryDto,
          ),
          PRODUCT_SERVICE,
          mockLoggerService,
          {
      ...
      it('should handle deletion with minimal response fields', async () => {
      ...
      it('should handle deletion with large ID values', async () => {
        ...
        const result = await service.deleteProductCategory(largeIdDto);

        expect(mockCallMicroservice).toHaveBeenCalledWith(
          mockProductClient.send(ProductPattern.DELETE_PRODUCT_CATEGORY, largeIdDto),
          PRODUCT_SERVICE,
          mockLoggerService,
          {
            timeoutMs: TIMEOUT_MS_DEFAULT,
      ...
      it('should handle deletion with zero ID', async () => {
        ...
        const zeroIdResponse: ProductCategoryResponse = {
          id: 0,
          categoryId: 0,
          productId: 0,
          createdAt: new Date('2024-01-01T00:00:00.000Z'),
        ...
        const zeroIdSuccessResponse: BaseResponse<ProductCategoryResponse> = {
          statusKey: StatusKey.SUCCESS,
          data: zeroIdResponse,
        };

        mockCallMicroservice.mockResolvedValueOnce(zeroIdResponse);
        mockBuildBaseResponse.mockReturnValue(zeroIdSuccessResponse);

        const result = await service.deleteProductCategory(zeroIdDto);
    ...
    describe('Error Handling', () => {
      it('should throw BadRequestException when microservice returns null', async () => {
        ...
        mockI18nService.translate.mockReturnValue('Failed to delete product category');

        await expect(service.deleteProductCategory(validDeleteProductCategoryDto)).rejects.toThrow(
          BadRequestException,
        );
        await expect(service.deleteProductCategory(validDeleteProductCategoryDto)).rejects.toThrow(
        ...
        expect(mockI18nService.translate).toHaveBeenCalledWith(
          'common.productCategory.action.delete.failed',
        );
      ...
      it('should handle not found errors', async () => {
        const notFoundError = new BadRequestException('Product category not found');
        mockCallMicroservice.mockRejectedValueOnce(notFoundError);
    ...
    describe('Edge Cases', () => {
      it('should handle negative ID values', async () => {
        const negativeDto: DeleteProductCategoryDto = {
          id: -1,
        };
        ...
        const result = await service.deleteProductCategory(negativeDto);
    ...
    describe('Type Safety and Validation', () => {
      ...
      it('should verify DTO structure and immutability', async () => {
        ...
        await service.deleteProductCategory(immutableDto);

        // Verify DTO structure
        expect(immutableDto).toHaveProperty('id');
        expect(Object.keys(immutableDto)).toEqual(['id']);
    ...
    describe('Microservice Integration', () => {
      it('should call microservice with correct pattern and parameters', async () => {
        const testDto: DeleteProductCategoryDto = {
          id: 25,
        };
        ...
        await service.deleteProductCategory(testDto);
        ...
        expect(mockCallMicroservice).toHaveBeenCalledWith(
          mockProductClient.send(ProductPattern.DELETE_PRODUCT_CATEGORY, testDto),
          PRODUCT_SERVICE,
          mockLoggerService,
          {
            timeoutMs: TIMEOUT_MS_DEFAULT,
      ...
      it('should use correct timeout and retry configuration', async () => {
        ...
        await service.deleteProductCategory(validDeleteProductCategoryDto);
      ...
      it('should handle microservice returning empty object', async () => {
        mockCallMicroservice.mockResolvedValueOnce({});
        mockBuildBaseResponse.mockReturnValue({
          statusKey: StatusKey.SUCCESS,
          data: {} as ProductCategoryResponse,
        });
