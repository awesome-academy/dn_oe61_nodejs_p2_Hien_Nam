...
import { NestResponse } from '@app/common/interfaces/request-cookie.interface';
...
describe('ApiGateway AuthService', () => {
  ...
  describe('twitterCallback', () => {
    ...
    it('should throw BadRequestException when user is null', async () => {
      await expect(
        service.twitterCallback(null as unknown as TwitterProfileDto),
    ...
    it('should throw BadRequestException when token signing fails', async () => {
      ...
      await expect(service.twitterCallback(baseProfile)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
