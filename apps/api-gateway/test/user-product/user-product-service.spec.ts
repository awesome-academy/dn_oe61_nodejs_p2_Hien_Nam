...
describe('UserProductService', () => {
  ...
  describe('listProductsForUser', () => {
    ...
    describe('successful scenarios', () => {
      it('should return products successfully with cache miss', async () => {
        ...
        expect(mockCacheService.getOrSet).toHaveBeenCalledWith(cacheKey, expect.any(Function), {
    ...
    describe('error scenarios', () => {
      it('should throw error when microservice returns empty array', async () => {
        const query: GetAllProductUserDto = { page: 1, pageSize: 50 };
        const cacheKey = 'user_products:page:1:pageSize:50';
      ...
      it('should throw error when cache returns null/undefined', async () => {
        const query: GetAllProductUserDto = { page: 1, pageSize: 50 };
        const cacheKey = 'user_products:page:1:pageSize:50';
        const errorMessage = 'Failed to get products';
  ...
  describe('getProductDetailForUser', () => {
    ...
    describe('service method issues', () => {
      it('should handle the bug where i18n.translate is called but not thrown when microservice result is null', async () => {
        ...
        const emptyProductDetail: UserProductDetailResponse = {
          id: 0,
          skuId: '',
          name: '',
          description: '',
        ...
        mockI18nService.translate.mockReturnValue(errorMessage);
        mockBuildBaseResponse.mockReturnValue(createMockDetailResponse(emptyProductDetail));
      ...
      it('should handle the bug where i18n.translate is called but not thrown when cache result is null', async () => {
        const dto: GetByIdProductDto = { skuId: 'SKU_CACHE_BUG' };
        const mockProductDetail = createMockProductDetail(1, 'SKU_CACHE_BUG');
        const productExistsResponse = createMockDetailResponse(mockProductDetail);
        const errorMessage = 'Failed to get product detail';
        ...
        mockCallMicroservice.mockResolvedValue(productExistsResponse);
        mockUpstashCacheService.generateKey.mockReturnValue('test_key');
        mockUpstashCacheService.getOrSet.mockResolvedValue(null);
        mockI18nService.translate.mockReturnValue(errorMessage);
        mockBuildBaseResponse.mockReturnValue(createMockDetailResponse(emptyProductDetail));
