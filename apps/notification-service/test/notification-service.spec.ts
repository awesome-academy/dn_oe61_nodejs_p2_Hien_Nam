...
describe('NotificationService', () => {
  ...
  describe('sendStatisticOderByMonthly', () => {
    const lang = 'en';
    const dto: GetStatisticByMonthRequest = { month: 9, year: 2025 };

    it('should send email jobs when microservice returns data and admins exist', async () => {
      const { callMicroservice } = await import('@app/common/helpers/microservices');
      const callMicroserviceMock = callMicroservice as jest.MockedFunction<typeof callMicroservice>;
      const mockStatistic = { totalOrders: 100 } as unknown as StatisticOrderByMonthResponse;
      callMicroserviceMock.mockResolvedValue(mockStatistic);
      jest
        .spyOn(service, 'fetchAllAdmins')
        .mockResolvedValue([{ id: 1, email: 'admin1@test.com', name: 'Admin One' }]);
      mockAddJobWithRetry.mockResolvedValue(undefined);
      await service.sendStatisticOrderMonthly(lang, dto);
      expect(callMicroserviceMock).toHaveBeenCalledWith(
        mockProductServiceClient.send(ProductPattern.GET_STATISTIC_BY_MONTH, dto),
        PRODUCT_SERVICE,
        mockLogger,
        {
          timeoutMs: TIMEOUT_MS_DEFAULT,
      ...
      expect(mockAddJobWithRetry).toHaveBeenCalledWith(
        mockEmailQueue,
        NotificationEvent.STATISTIC_ORDER_MONTHLY,
    ...
    it('should fallback to "Failed to get statistic data" when microservice returns null', async () => {
      const { callMicroservice } = await import('@app/common/helpers/microservices');
      const callMicroserviceMock = callMicroservice as jest.MockedFunction<typeof callMicroservice>;
      callMicroserviceMock.mockResolvedValue(null);
      jest
        .spyOn(service, 'fetchAllAdmins')
        .mockResolvedValue([{ id: 1, email: 'admin2@test.com', name: 'Admin Two' }]);

      await service.sendStatisticOrderMonthly(lang, dto);

      expect(mockLogger.error).toHaveBeenCalledWith('Failed to get statistic data - after retries');
      expect(mockAddJobWithRetry).toHaveBeenCalledWith(
        mockEmailQueue,
        NotificationEvent.STATISTIC_ORDER_MONTHLY,
        expect.objectContaining({
          data: 'Failed to get statistic data',
        }),
      );
    });

    it('should log error and not send jobs when no admins found', async () => {
      const { callMicroservice } = await import('@app/common/helpers/microservices');
      const callMicroserviceMock = callMicroservice as jest.MockedFunction<typeof callMicroservice>;
      callMicroserviceMock.mockResolvedValue({ totalOrders: 50 });
      jest.spyOn(service, 'fetchAllAdmins').mockResolvedValue([]);

      await service.sendStatisticOrderMonthly(lang, dto);

      expect(mockLogger.error).toHaveBeenCalledWith('Failed to get admin data - after retries');
      expect(mockLogger.error).toHaveBeenCalledWith(
        `Failed to send statistic order by month ${dto.month}`,
        `Caused:: Admin is undefined`,
      );
      expect(mockAddJobWithRetry).not.toHaveBeenCalled();
    });

    it('should continue even if some jobs fail', async () => {
      const { callMicroservice } = await import('@app/common/helpers/microservices');
      const callMicroserviceMock = callMicroservice as jest.MockedFunction<typeof callMicroservice>;
      callMicroserviceMock.mockResolvedValue({ totalOrders: 80 });

      jest.spyOn(service, 'fetchAllAdmins').mockResolvedValue([
        { id: 1, email: 'admin3@test.com', name: 'Admin Three' },
        { id: 2, email: 'admin4@test.com', name: 'Admin Four' },
      ]);
      mockAddJobWithRetry
        .mockRejectedValueOnce(new Error('Job failed'))
        .mockResolvedValueOnce(undefined);
      await expect(service.sendStatisticOrderMonthly(lang, dto)).resolves.not.toThrow();
      expect(mockAddJobWithRetry).toHaveBeenCalledTimes(2);
    });
  });
});
