...
// Import constants directly since cache.constants file doesn't exist
const DEFAULT_CACHE_TTL_1H = 3600;
const MAX_DATA_BYTES = 104857600; // 100MB
...
describe('CacheService', () => {
  ...
  describe('constructor', () => {
    ...
    it('should create Redis instance with correct configuration', () => {
      // This test verifies that the service is created successfully with config
      // The actual Redis constructor call is mocked by our manual mock
      ...
      const testService = new CacheService(freshMockConfigService as unknown as ConfigService);

      expect(testService).toBeDefined();
      expect(testService).toBeInstanceOf(BaseCacheService);
      expect(freshMockConfigService.get).toHaveBeenCalledTimes(2);
      expect(freshMockConfigService.get).toHaveBeenNthCalledWith(1, 'REDIS_HOST', 'localhost');
      expect(freshMockConfigService.get).toHaveBeenNthCalledWith(2, 'REDIS_PORT');
    });
    ...
    it('should handle Redis connect event', function (this: void) {
      ...
      expect(serviceWithLogger.logger.log).toHaveBeenCalledWith('Redis connection established');
    });
    ...
    it('should handle Redis close event', function (this: void) {
      ...
      expect(serviceWithLogger.logger.warn).toHaveBeenCalledWith('Redis connection closed');
    });
  });
  ...
  describe('onModuleDestroy', () => {
    it('should close Redis connection gracefully', async function (this: void) {
      ...
      const closeRedisConnectionSpy = jest.spyOn(
        service as CacheService & { closeRedisConnection: () => Promise<void> },
